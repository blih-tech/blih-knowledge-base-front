...
function InitiativeDetail({ item, onBack, onEdit, onDelete, canManage, onStatusChange }: {
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            {canManage && (
              <div className="flex items-center gap-2">
                <select value={item.status} onChange={(e) => onStatusChange(e.target.value as InitiativeStatus)}
...
export default function AdminInitiativesPage() {
  ...
  const { createInitiative: createMut, updateInitiative: updateMut, deleteInitiative: deleteMut, updateStatus } = useInitiativeMutations();
  ...
  const handleStatusChange = async (status: InitiativeStatus) => {
    if (!selected) return;
    const updated = await updateStatus.mutateAsync({ id: selected._id, status });
    setSelected(updated);
  };
  ...
  if (view === "detail" && selected) {
    return (
      <InitiativeDetail item={selected} onBack={() => { setSelected(null); setView("list"); }}
        ...
        canManage={canManage || selected.author?._id === user?._id} onStatusChange={handleStatusChange} />
    );
  }
